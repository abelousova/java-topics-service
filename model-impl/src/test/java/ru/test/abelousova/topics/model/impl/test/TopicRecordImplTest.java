...
import ru.test.abelousova.topics.model.impl.TopicRecordImpl;
...
@ContextConfiguration(classes = TopicRecordImplTest.ContextConfiguration.class, loader=AnnotationConfigContextLoader.class)
public class TopicRecordImplTest {
    private static final File TEST_DIR = new File("./src/test/resourses/test_dir");

    @Configuration
    public static class ContextConfiguration {
        @Bean
        public TopicRecordImpl record() {
            return new TopicRecordImpl("2015-11-12-12-35-26",
                    new File(TEST_DIR, "topic6" + File.separator + "history"));
        }

        @Bean
        public TopicRecordImpl emptyRecord() {
            return new TopicRecordImpl("2015-11-11-17-15-20",
                    new File(TEST_DIR, "topic1" + File.separator + "history"));
        }
    }

    @Resource(name = "record")
    private TopicRecordImpl record;

    @Resource(name = "emptyRecord")
    private TopicRecordImpl emptyRecord;
